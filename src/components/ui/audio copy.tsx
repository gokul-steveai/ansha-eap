...
const Audio = ({
  ...
  return (
    <div
      className={cn(
        "bg-white rounded-xl shadow-md w-full rounded-lg overflow-hidden",
        className
      )}
    >
      {/* === DEFAULT PLAYER === */}
 ...
 >
   ...
   <div
     className={cn(
       "p-4 opacity-0 pointer-events-none flex-1 w-full h-full flex flex-col justify-between min-w-0 absolute top-0 left-0 audio-player-overlay transition-opacity duration-300",
       "group-hover:opacity-100 group-hover:pointer-events-auto", // show on hover
       !isPlaying && "opacity-100 pointer-events-auto" // always show if not playing
     )}
   >
     ...
     <div className="mt-auto flex gap-3 w-full flex-col">
       ...
       <div className="text-white flex items-center gap-2">
         <Button
           onClick={togglePlay}
           variant="ghost"
           className="!pl-0 rounded-full hover:bg-black/10"
           size="icon"
         >
           {isPlaying ? (
