...
import {
    ...
    ShortCourse,
    updateShortCourse,
} from "@/serverActions/crudShortCourses";
...
type FormShortCourseProps = {
    data?: ShortCourse;
    onSubmitSuccess?: (data: ShortCourse) => void;
    hideSubmitButton?: boolean;
    submitForm?: (submitFn: () => void) => void;
};

export default function FormShortCourse({
    data,
    onSubmitSuccess,
    hideSubmitButton = false,
    submitForm,
}: FormShortCourseProps) {
    const {
        register,
        handleSubmit,
    ...
    const onSubmit = async (data: ShortCourse) => {
        try {
            let result;
            if (data?.id) {
                result = await updateShortCourse(data.id, data);
            } else {
                result = await createShortCourse(data);
            }
