...
const PractionerSingleMoBile = async ({ params }: PractionerSingleProps) => {
...
export default PractionerSingleMoBile;
...
async function Single({ id }: { id: string }) {
  ...
  const professionalName =
    `${practionerData.title} ${practionerData.first_name} ${practionerData.last_name}`.trim();

  return (
    <div className="flex gap-6 h-full overflow-auto relative">
      <Tabs defaultValue="book" className="h-full">
        <TabsList className="w-full bg-[#e8eaed] sticky top-[0px]">
          <TabsTrigger className="flex-1" value="profile">Profile</TabsTrigger>
          <TabsTrigger className="flex-1" value="book">Book</TabsTrigger>
        </TabsList>
          ...
          <Container className="card">
            ...
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: practionerData.description ?? "",
              }}
            />
          </Container>
        </TabsContent>
